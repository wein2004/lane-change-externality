...
```bash
python tests/test_detection.py        # 23 checks on synthetic trajectories
...
docs/      decision log and methodology — why, not just what
...
1. **Small truck sample.** 54 truck events against 2,418 car events. The
...
2. **Short observation windows.** I-80's section is ~500 m, US-101's ~640 m.
   Recovery times are right-censored at 10 s, so the reported figure is a
   lower bound.
3. **2005 US freeway data.** Vehicle mix, engine performance and driver
   ...
   on them.
4. **Observational, not experimental.** Matching and regression remove the
